defmodule Synthex.Oscillator.Triangle do
  def init(opts) do
    ...
    two_on_pi = 2/pi
    %{phase_delta: phase_delta, two_on_pi: two_on_pi, period: round(rate / frequency)}
  end
  ...
  defp do_get_sample(phase, pi, phase_offset) when phase < pi, do: -1 + phase_offset
  defp do_get_sample(_phase, _pi, phase_offset), do: 3 - phase_offset
end